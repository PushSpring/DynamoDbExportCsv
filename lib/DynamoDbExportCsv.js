...
const safeParse = potentialJSONString => {
    if (_.isString(potentialJSONString) && !_.isEmpty(potentialJSONString)) {
        try {
            return JSON.parse(potentialJSONString);
        } catch (error) {
            /* Ignore an error and just pass the original input */
        }
    }
    return potentialJSONString;
};

function DynamoDBExportCSV(awsAccessKeyId, awsSecretAccessKey, awsRegion, awsOptions) {
    // Events this emits
    var infoEvent = 'info';
    ...
    var _awsOptions = safeParse(awsOptions);
    var _args = [];
    ...
    if (_awsOptions) {
        _args.push('--awsoptions');
        _args.push(JSON.stringify(_awsOptions));
    }
    AWS.config.update(config);

    var dynamodb = new AWS.DynamoDB(_awsOptions.dynamodb || { maxRetries: 20 });
    var s3 = new AWS.S3(_awsOptions.s3);
