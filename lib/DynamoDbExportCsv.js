...
function DynamoDBExportCSV(awsAccessKeyId, awsSecretAccessKey, awsRegion) {
    ...
    function writeTableToCsv(table, columns, totalSegments, segment, compressed, filesize, s3Bucket, s3Path, filterExpression, expressionAttributeNames, expressionAttributeValues, callback) {
        if (typeof(filterExpression) === 'function') { // for backward compability
        ...
        if (filterExpression) {
            ...
            if (expressionAttributeNames) {
                query.ExpressionAttributeNames = JSON.parse(expressionAttributeNames);
            }
            if (expressionAttributeValues) {
                query.ExpressionAttributeValues = JSON.parse(expressionAttributeValues);
            }
        }
    ...
    this.exportTable = function exportTable(table, columns, totalSegments, compressed, filesize, s3Bucket, s3Path, filterExpression, expressionAttributeNames, expressionAttributeValues, callback) {
        if (typeof(filterExpression) === 'function') { // for backward compability
            callback = filterExpression;
            filterExpression = undefined;
        }
        ...
        async.series([
            ...
            function (done) {
                ...
                if (filterExpression) {
                    allWorkerArgs.push('--filter-expression');
                    allWorkerArgs.push(filterExpression);
                    if (expressionAttributeNames) {
                        allWorkerArgs.push('--expression-attribute-names');
                        allWorkerArgs.push(expressionAttributeNames);
                    }
                    if (expressionAttributeValues) {
                        allWorkerArgs.push('--expression-attribute-values');
                        allWorkerArgs.push(expressionAttributeValues);
                    }
                }
    ...
    // Public export table function
    this.exportTableWorker = function exportTableWorker(table, columns, totalSegments, segment, compressed, filesize, s3Bucket, s3Path, filterExpression, expressionAttributeNames, expressionAttributeValues, callback) {
        ...
        async.series([
            ...
            // Scan the table
            function (done) {
                writeTableToCsv(table, columns, totalSegments, segment, compressed, filesize, s3Bucket, s3Path, filterExpression, expressionAttributeNames, expressionAttributeValues, done);
            }
        ], callback);
    };
